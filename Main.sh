#!/usr/bin/env bash
# ===== SAFETY: explicit enable required for real network actions =====
# Prevent accidental real runs. To allow real execution, run:
#   export ENABLE_REAL_RUN=1
# and then run the script. Otherwise the script will run in SAFE_MODE.
if [ "${ENABLE_REAL_RUN:-}" != "1" ]; then
  echo "REAL RUN DISABLED — enable only in an isolated, authorized test environment:"
  echo "  export ENABLE_REAL_RUN=1"
  echo "Then run: sudo ./Main.sh"
  SAFE_MODE=1
else
  SAFE_MODE=0
fi
